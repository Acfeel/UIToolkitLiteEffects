Shader "Hidden/Acfeel/UIToolkitLiteEffects"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float3 ApplyHue(float3 rgb, float hueShift)
            {
                float angle = (hueShift - 0.5) * 6.28318;
                float cosH = cos(angle);
                float sinH = sin(angle);

                return float3(
                    rgb.x * (0.299 + 0.701 * cosH + 0.168 * sinH) + rgb.y * (0.587 - 0.587 * cosH + 0.330 * sinH) + rgb.z * (0.114 - 0.114 * cosH - 0.497 * sinH),
                    rgb.x * (0.299 - 0.299 * cosH - 0.328 * sinH) + rgb.y * (0.587 + 0.413 * cosH + 0.035 * sinH) + rgb.z * (0.114 - 0.114 * cosH + 0.292 * sinH),
                    rgb.x * (0.299 - 0.300 * cosH + 1.25 * sinH) + rgb.y * (0.587 - 0.588 * cosH - 1.05 * sinH) + rgb.z * (0.114 + 0.886 * cosH - 0.203 * sinH)
                );
            }
